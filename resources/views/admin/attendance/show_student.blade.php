...
<div class="max-w-6xl mx-auto mt-10 px-4">
    ...
    <br>
    <!-- FILTER KELAS -->
    <form method="GET" action="{{ route('admin.attendance.show') }}" class="mb-6 flex gap-3">
    ...
    <form method="POST" action="{{ route('admin.attendance.update') }}">
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    @foreach($users as $i=>$s)

                    @php
                    $a = $attendances[$s->id] ?? null;
                    @endphp

                    <tr class="{{ $i%2?'bg-gray-50':'' }} hover:bg-indigo-50">
        ...
        <div class="text-right mt-6">
            <button
                class="px-8 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow font-semibold">
                💾 Simpan Perubahan
            </button>
        </div>
